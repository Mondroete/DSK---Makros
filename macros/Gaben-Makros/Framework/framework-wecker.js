// ==========================================
// 🌙 LUNAS WECKER (Der Kompendium-Bibliothekar)
// ==========================================
(async () => {
    // --- 1. FRAMEWORK WECKEN ---
    ...
    for (let actor of alleAkteure) {
        for (let effect of actor.effects) {
            ...
            // Nur Effekte mit Ahnen-Namen in Klammern prüfen (überspringt z.B. "Schmerz")
            if (!eName.includes("(")) continue;

            // "Schwarze Katze (Aphasma)" -> "Schwarze Katze"
            ...
            if (macro) {
                ...
                const codeText = macro.command; // Liest den Code als Textdatei!

                // Magischer Scanner: Sucht den Abbruch-Text im Code
                const match = codeText.match(/EffektEnde\.Register\s*\(\s*[^,]+,\s*[^,]+,\s*(["'`])(.*?)\1\s*\)/);
                const beschreibung = match ? match[2] : `Die Kraft von ${eName} verblasst.`;

                // EffektEnde automatisch mit dem Original-Text registrieren
                DSK_FW.EffektEnde.Register(effect.id, eName, beschreibung);

                // --- 5. SICHERHEITSNETZ FÜR LIVE-SYNC TIER-GABEN ---
                if (basisName === "Fledermausleib" && !globalThis._dskFledermausSystem) {
                    ...
                    Hooks.on("deleteActiveEffect", async (eff) => {
                        ...
                        if ((eff.name || eff.label).includes("Fledermausleib")) {
                ...
                if (basisName === "Mäusemeister" && !globalThis._dskMaeusemeisterSystem) {
                    ...
                    Hooks.on("deleteActiveEffect", async (eff) => {
                        if (!game.user.isGM) return; 
                        if ((eff.name || eff.label).includes("Mäusemeister")) {
                            const actorUuid = eff.parent?.uuid;
                            if (!actorUuid) return;
